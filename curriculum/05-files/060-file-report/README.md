# 060 · 生成词频报告文件

## 学习目标

组合文本输入、统计和 JSON 输出。

## 前置知识

先完成第 059 题及其前置练习；本题复用此前学过的知识。

## 先学一点

`text.split()` 按连续空白分词。此处的“词”仅指空白分隔的片段，不做中文分词或标点清洗。

## 任务与约定

完成 `word_report(source, destination)`，读取现有 UTF-8 文本，按空白分词，转小写并计数。将词频字典写入另一个 UTF-8 JSON 文件，覆盖旧内容，同时返回字典。两个路径不同。

除题目明确要求外，不修改传入的可变数据，不额外打印内容。只验收约定范围内的输入，不要求猜测未说明的规则。

## 示例

输入文本 `Py py\nWeb` → 返回并保存 `{"py": 2, "web": 1}`。

## 开始编写

编辑 `main.py`。起始代码位于 [starter](./starter/)，测试位于 [test/test_solution.py](./test/test_solution.py)。起始代码尚未完成，出现未通过是正常反馈。

## 分层提示

<details>
<summary>提示 1：思路</summary>

先得到统计字典，再写文件。

</details>

<details>
<summary>提示 2：实现方向</summary>

空文件应生成 JSON 空对象。

</details>

## 如何验收

提交后检查题目约定中的正常行为及适用边界。测试针对结果，不要求与参考实现写法相同。失败时先对照测试输入和约定，再修改代码。
