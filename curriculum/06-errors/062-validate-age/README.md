# 062 · 主动拒绝非法年龄

## 学习目标

使用 raise 表达输入约束。

## 前置知识

先完成第 061 题及其前置练习；本题复用此前学过的知识。

## 先学一点

`raise ValueError("原因")` 告诉调用者输入值不符合约定。`type(value) is int` 可精确判断整数，避免把 bool 当成年龄。

## 任务与约定

完成 `validate_age(age)`。只有类型恰好为 int 且位于 0–120（含端点）时返回原值；其他情况都抛出 ValueError，错误文字不限。

除题目明确要求外，不修改传入的可变数据，不额外打印内容。只验收约定范围内的输入，不要求猜测未说明的规则。

## 示例

`validate_age(18)` → `18`；`validate_age(True)` 抛出 `ValueError`。

## 开始编写

编辑 `main.py`。起始代码位于 [starter](./starter/)，测试位于 [test/test_solution.py](./test/test_solution.py)。起始代码尚未完成，出现未通过是正常反馈。

## 分层提示

<details>
<summary>提示 1：思路</summary>

类型和取值范围都需要检查。

</details>

<details>
<summary>提示 2：实现方向</summary>

先检查类型，再比较大小。

</details>

## 如何验收

提交后检查题目约定中的正常行为及适用边界。测试针对结果，不要求与参考实现写法相同。失败时先对照测试输入和约定，再修改代码。
