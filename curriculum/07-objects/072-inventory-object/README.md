# 072 · 保护库存快照

## 学习目标

避免通过返回值泄漏内部可变状态。

## 前置知识

先完成第 071 题及其前置练习；本题复用此前学过的知识。

## 先学一点

如果直接返回内部字典，调用者能绕开方法修改库存。返回副本能保护本题的整数值映射。

## 任务与约定

实现 `Inventory(stock)`，stock 为商品名到非负整数的字典。复制初始数据；`add(name, quantity)` 接受非负整数数量并累加，返回新数量；`snapshot()` 返回独立字典。name 为字符串。

除题目明确要求外，不修改传入的可变数据，不额外打印内容。只验收约定范围内的输入，不要求猜测未说明的规则。

## 示例

初始 `{"笔": 2}`，`add("笔", 3)` → `5`。

## 开始编写

编辑 `main.py`。起始代码位于 [starter](./starter/)，测试位于 [test/test_solution.py](./test/test_solution.py)。起始代码尚未完成，出现未通过是正常反馈。

## 分层提示

<details>
<summary>提示 1：思路</summary>

构造时复制，返回快照时也复制。

</details>

<details>
<summary>提示 2：实现方向</summary>

新商品的原数量默认为 0。

</details>

## 如何验收

提交后检查题目约定中的正常行为及适用边界。测试针对结果，不要求与参考实现写法相同。失败时先对照测试输入和约定，再修改代码。
