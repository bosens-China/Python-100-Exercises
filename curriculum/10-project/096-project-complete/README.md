# 096 · 任务清单：完成任务

## 学习目标

实现幂等的状态操作。

## 前置知识

先完成第 095 题及其前置练习；本题复用此前学过的知识。

## 先学一点

幂等表示重复执行同一操作得到相同的最终状态。“设为完成”是幂等操作，“反转状态”则不是。

## 任务与约定

在 Store 中实现 `complete(identifier)`。ID 必须恰好为 int 且存在，否则 KeyError。将对应 done 设为 True，返回记录副本；重复完成仍保持 True。不改变其他记录。保留 add 与 list。

除题目明确要求外，不修改传入的可变数据，不额外打印内容。只验收约定范围内的输入，不要求猜测未说明的规则。

## 示例

完成 ID 1 两次后，`list(True)` 中仍是一条完成记录。

## 开始编写

编辑 `domain.py`, `store.py`, `main.py`。起始代码位于 [starter](./starter/)，测试位于 [test/test_solution.py](./test/test_solution.py)。起始代码尚未完成，出现未通过是正常反馈。

## 分层提示

<details>
<summary>提示 1：思路</summary>

先验证存在性，再赋值 True。

</details>

<details>
<summary>提示 2：实现方向</summary>

不要使用 not 反转 done。

</details>

## 如何验收

提交后检查题目约定中的正常行为及适用边界。测试针对结果，不要求与参考实现写法相同。失败时先对照测试输入和约定，再修改代码。
