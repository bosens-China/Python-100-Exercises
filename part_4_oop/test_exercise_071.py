...
class TestReprMethod(unittest.TestCase):
    def test_repr_output(self):
        ...
        fido = Dog("Fido", 5)
        self.assertEqual(repr(fido), "Dog(name='Fido', age=5)")

    def test_repr_in_container(self):
        """测试在容器中对象的 __repr__ 是否被正确调用。"""
        buddy = Dog("Buddy", 2)
        dog_list = [buddy]
        self.assertEqual(str(dog_list), "[Dog(name='Buddy', age=2)]")
    ...
    def test_eval_repr(self):
        ...
        self.assertIsInstance(recreated_lucy, Dog)
        self.assertEqual(recreated_lucy.name, lucy.name)
        self.assertEqual(recreated_lucy.age, lucy.age)
        # 验证两个对象的 __str__ 输出也相同
        self.assertEqual(str(recreated_lucy), str(lucy))
